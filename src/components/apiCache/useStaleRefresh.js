/* ***************************************************************
  Code found at:
   https://www.toptal.com/react-hooks/stale-while-revalidate
*/   

import { useState, useEffect, useRef } from "react";
import isEqual from "lodash/isEqual";
const CACHE = {};

export default function useStaleRefresh(fn, args, bSkip, urlHeaders, defaultValue = []) {
  const prevArgs = useRef(null);
  const [data, setData] = useState(defaultValue);
  const [isLoading, setLoading] = useState(true);

  useEffect(() => {
    // args is an object so deep compare to rule out false changes
    if (isEqual(args, prevArgs.current) || bSkip) {
      return;
    }
    // cacheID is how a cache is identified against a unique request
    const cacheID = hashArgs(fn.name, ...args);
    // look in cache and set response if present
    if (CACHE[cacheID] !== undefined) {
      setData(CACHE[cacheID]);
      setLoading(false);
      console.log("getting data from CACHE...("+cacheID+")");
    } else {
      // else make sure loading set to true
      setLoading(true);
        console.log("fetching new data...("+cacheID+")");
        // fetch new data
        fn(...args, urlHeaders).then(newData => {
          CACHE[cacheID] = newData;
          setData(newData);
          setLoading(false);
        });
    }
  }, [urlHeaders, bSkip, args, fn]);

  useEffect(() => {
    prevArgs.current = args;
  });

  return [data, isLoading];
}

function hashArgs(...args) {
  return args.reduce((acc, arg) => stringify(arg) + ":" + acc, "");
}

function stringify(val) {
  return typeof val === "object" ? JSON.stringify(val) : String(val);
}
